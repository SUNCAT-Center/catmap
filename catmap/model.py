...
class ReactionModel:
            ...
            if var in self._classes:
                ...
                try:
                    if locs[var]:
                        ...
                        # _temp = __import__(pyfile,globals(),sublocs, [locs[var]]) #no reason to
                        # mess with globals() unless we have to.
    ...
    def parse_elementary_rxns(self, equations):
        ...
        for rxn_index, rxn in enumerate(equations):
            ...
            for key in ['initial_state', 'transition_state', 'final_state']:
                ...
                if state_str:
                    state_strings = [si for si in state_str.split() if si]
                    # echem transition state syntax parsing. generate echem transition state
                    # from TS like "^0.6eV_a"
                    if key == 'transition_state' and len(state_strings) == 1 and state_strings[0].startswith('^'):
                        try:
                            ...
                            echem_TS_name = '-'.join(["echemTS",
                                                      str(rxn_index), barrier]) + "_" + site
    ...
    def model_summary(self, summary_file='summary.tex'):
        ...
        for spec in self.gas_names:
            ...
            reference = '\parbox[t]{4cm}{' + \
                cleanstring(self.species_definitions[spec]['formation_energy_source']) + '}'
    ...
    def generate_echem_TS(self):
        ...
        for echem_TS in self.echem_transition_state_names:
            ...
            self.species_definitions[echem_TS]['formation_energy_source'] = [
                "Generated by CatMAP"] * len(self.surface_names)
            ...
            # Someone may want to change this to be user-specified at some point
            self.species_definitions[echem_TS]['n_sites'] = 1
            self.species_definitions[echem_TS]['composition'] = {
                'H': 1}  # placeholder composition - should be unimportant
