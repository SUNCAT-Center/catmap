...
class SteadyStateSolver(MeanFieldSolver):
    ...
    def get_interacting_coverages(self, rxn_parameters, c0=None,
        ...
        if interaction_strength is not None:
            if len(rxn_parameters) == n_tot + n_tot**2:
                rxn_parameters = rxn_parameters[
                    :n_tot] + [pi * interaction_strength for pi in rxn_parameters[-n_tot**2:]]
    ...
    def bisect_interaction_strength(self, rxn_parameters, valid_strength, valid_coverages, target_strength, max_bisections, findrootArgs={}):
        ...
        while n_bisects <= max_bisections:
            ...
            new_params = rxn_parameters[:n_tot] + \
                [pi * new_strength for pi in rxn_parameters[-n_tot**2:]]
            try:
    ...
    def get_initial_coverage(self, rxn_parameters):
        ...
        if not self.atomic_reservoir_dict:
            ...
            boltz_cvgs = [[0] * len(self.adsorbate_names)]
    ...
    def compile(self):
        ...
        if not self._compiled:
            ...
            energy_expressions_noderivs = '\n    '.join(
                self.reaction_energy_equations(adsorbate_interactions=False))
            energy_expressions_derivs = '\n    '.join(
                self.reaction_energy_equations(adsorbate_interactions=True))

            templates['rate_constants_no_derivatives'] = Template(templates['rate_constants']).safe_substitute({
                'elementary_step_energetics': energy_expressions_noderivs})
            templates['rate_constants_with_derivatives'] = Template(templates['rate_constants']).safe_substitute({
                'elementary_step_energetics': energy_expressions_derivs})
            ...
            # pre-substitute the interaction function into rate_constants (needed
            # because its nested 2 levels)
            indented = indent_string(
                templates[self.adsorbate_interaction_model + '_interaction_function'], 1)
            ...
            if self.optimize_analytical_expressions:
                ...
                test_params = [self._mpfloat(random.random())
                               for a in self.adsorbate_names + self.transition_state_names]
