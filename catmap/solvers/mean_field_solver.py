...
class MeanFieldSolver(SolverBase):
    ...
    def get_rate_control(self, rxn_parameters):
        ...
        try:
            ...
            dRdG = numerical_jacobian(self.get_turnover_frequency,
                                      rxn_parameters, self._matrix, eps, diff_idxs=diff_idxs)
        except ValueError, strerror:
            resid = str(strerror).rsplit('=', 1)[1]
            resid = resid.replace(')', '')
            resid.strip()
            self.log('jacobian_fail', resid=resid)
            dRdG = np.zeros((len(self.gas_names), len(
    ...
    def get_interacting_energies(self, rxn_parameters):
        ...
        E_int = self.interaction_function(
            cvg, energies, eps_vector, self.thermodynamics.adsorbate_interactions.interaction_response_function, False, False)[1]
        return E_int
    ...
    def get_apparent_activation_energy(self, rxn_parameters, epsilon=1e-10):
        ...
        # don't overwrite them, if temperature is a descriptor
        descriptors = list(self._rxm.mapper._descriptors)
        ...
        for i, gas in enumerate(self.gas_names):
            barriers_i = []
            # this will fail if any of the TOFs are 0.
            dlnTOF = mp.log(new_tofs[i]) - mp.log(current_tofs[i])
    ...
    def substitutions_dict(self):
        ...
        subdict['n_transition_states'] = 'n_transition_states = ' + \
            str(len(self.transition_state_names))
            ...
            for j, ads_j in enumerate(self.adsorbate_names):
                ...
                for k, rxn in enumerate(self.elementary_rxns):
                    ...
                    if rxnOrder:
                        ...
                        if adsorbate_interactions == True:
                            dfRate_string = self.rate_equation_term(
                                rxn[0], '(kfkBT[' + str(k) + '])*dEf[' + str(k) + '][' + str(j) + ']')
                            drRate_string = self.rate_equation_term(
                                rxn[-1], '(krkBT[' + str(k) + '])*dEr[' + str(k) + '][' + str(j) + ']')
    ...
    def reaction_energy_equations(self, adsorbate_interactions=True):
        ...
        for i, rx in enumerate(self.elementary_rxns):
            ...
            if adsorbate_interactions == True:
                txt += 'dG_IS[' + str(i) + '] = element_wise_addition([' + \
                    ' , '.join(species_strings(IS, 'dGs', False, 'list')) + '])\n    '
                txt += 'dG_FS[' + str(i) + '] = element_wise_addition([' + \
                    ' , '.join(species_strings(FS, 'dGs', False, 'list')) + '])\n    '
            if TS:
                TS_strings = species_strings(TS, 'Gf')
                txt += 'G_TS[' + str(i) + '] = ' + ' + '.join(species_strings(TS, 'Gf')) + '\n    '
                if adsorbate_interactions == True:
                    txt += 'dG_TS[' + str(i) + '] = element_wise_addition([' + \
                        ' , '.join(species_strings(TS, 'dGs', False, 'list')) + '])\n    '
            else:
                txt += 'G_TS[' + str(i) + '] = max([G_IS[' + str(i) + \
                    '],G_FS[' + str(i) + ']])' + '\n    '
                if adsorbate_interactions == True:
    ...
    def get_empty_site_cvgs(self):
        ...
        for site, eq in zip(self._rxm.site_names, site_eqs):    # get all the site names in the model
            ...
            if 'mpf' in eq_list[0]:
                ...
                res = float(self._coverage[
                            int(eq_list[0][eq_list[0].index('[') + 1:eq_list[0].index(']')])])
    ...
    def get_elem_ec(self, rxn_num, rxn_parameters, direction):  # rxn_num based on zero-index
        ...
        """
        if direction == 'kf':
            eq = self.rate_equation_term(self._rxm.elementary_rxns[rxn_num][
                                         0], 'kf[' + str(rxn_num) + ']')
        elif direction == 'kr':
            eq = self.rate_equation_term(self._rxm.elementary_rxns[
                                         rxn_num][-1], 'kr[' + str(rxn_num) + ']')
        ...
        for species in eq_list:
            if 'kf' in species:
                parsed_results *= float(
                    rate_constants[int(species[species.index('[') + 1:species.index(']')])])
            elif 'kr' in species:
                parsed_results *= float(rate_constants[len(rate_constants) /
                                                       2 + int(species[species.index('[') + 1:species.index(']')])])
            elif 'p' in species:
                parsed_results *= float(
                    pressure[int(species[species.index('[') + 1:species.index(']')])])
            elif 'theta' in species:
                parsed_results *= float(
                    coverages[int(species[species.index('[') + 1:species.index(']')])])
            else:
    ...
    def get_directional_rates(self, rxn_parameters):
        ...
        directional_rates = [self.get_elem_ec(i, rxn_parameters, 'kf') for i in range(
            num_rxns)] + [self.get_elem_ec(i, rxn_parameters, 'kr') for i in range(num_rxns)]
