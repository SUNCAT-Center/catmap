...
dG_Pt = {
    # 1.7 + 2.33 - 4.99,
    '7': ads_energy_dict['O_b'] + ads_energy_dict['O_a'] - ads_energy_dict['O2_a'],
    # 0.75 + 2.33 - 3.91,
    '10': ads_energy_dict['O_b'] + ads_energy_dict['OH_a'] - ads_energy_dict['OOH_a'],
    # 0.75 + 1.1 - 3.33,
    '12': ads_energy_dict['OH_b'] + ads_energy_dict['OH_a'] - ads_energy_dict['H2O2_a'],
}
...
header = ['surface_name', 'site_name', 'species_name', 'formation_energy',
...
for i, label in enumerate(labels):
    ...
    for ads in ads_energy_dict.keys():
        ...
        if facet == 'dl':
            ...
            lines.append([surface, facet, ads_species, str(
                ads_energy), 'fcc', '[]', '[]', 'Hansen 2014'])
        elif ads_species != '*':
    ...
    for chem_TS in chemical_TS_energy_dict.keys():
        dG = (tempdict[TS_FSs[chem_TS][0]] + tempdict[TS_FSs[chem_TS][1]]) - \
            tempdict[TS_ISs[chem_TS]]
        e_TS_Pt = chemical_TS_energy_dict[chem_TS]
        energy = tempdict[TS_ISs[chem_TS]] + e_TS_Pt + \
            chemical_TS_gamma_dict[chem_TS] * (dG - dG_Pt[chem_TS])  # Eqn (17)
